...
async function grindLaunchSalt(name: string, symbol: string): Promise<{ salt: Hex; address: Address }> {
  ...
  for (let counter = 0; counter <= 0xffff_ffff; counter += 1) {
    ...
    if ((BigInt(`0x${token.slice(-4)}`) & mask) === (suffix & mask)) {
      console.log(`  -> vanity salt found after ${counter.toLocaleString()} attempts`)
      return { salt, address: token }
    }
    // Yield periodically so Telegram/HTTP remain responsive during a long grind.
...
type Stats = Record<Action, number> & {
  cycles: number; spentUsd: number; nativeGasUsdc: number; fliptUsdc: number;
  creatorFeesUsd: number; startedAt: string; updatedAt: string
}
...
// Runner health is separate from pause state. Telegram /status and Render /health
// can now tell whether the trading loop is actually alive.
let runnerAlive = false
let runnerStage = 'starting'
let lastRunnerError: string | undefined
let lastRunnerErrorAt: string | undefined
let lastRunnerActivityAt = new Date().toISOString()
let consecutiveErrors = 0

function touchRunner(stage: string) {
  runnerStage = stage
  lastRunnerActivityAt = new Date().toISOString()
}
...
function runnerState() {
  ...
  if (!runnerAlive) return 'not-running'
  if (paused) return 'paused'
  if (consecutiveErrors > 0) return 'recovering'
  return 'running'
}
...
async function loadStats(): Promise<Stats> {
  ...
  try { return { ...blank, ...(JSON.parse(await readFile(statsPath, 'utf8')) as Partial<Stats>) } }
  catch (error: unknown) {
...
async function registerTelegramCommands() {
  ...
  } catch (error) { console.error('Telegram command registration failed:', error) }
}
...
function telegramStatus() {
  ...
  const lines = [
    ...
    `State: ${runnerState()}`,
    `Stage: ${runnerStage}`,
    `Runner alive: ${runnerAlive ? 'yes' : 'no'}`,
  ...
  if (lastRunnerError) {
    lines.push(`Last error: ${lastRunnerError}`)
    if (lastRunnerErrorAt) lines.push(`Error at: ${lastRunnerErrorAt}`)
  }
  return lines.join('\n')
}
    ...
    try {
      ...
      for (const update of body.result) {
        ...
        if (command === '/start' || command === '/resume') {
          paused = false
          if (!runnerAlive) {
            await telegramNotify(`Runner is not alive; resume flag was set but the process needs a Render restart.\n\n${telegramStatus()}`)
          } else {
            touchRunner(runnerStage === 'paused' ? 'resuming' : runnerStage)
...
async function loadTrackedTokens() {
  try {
    ...
    const seen = new Set<Address>(); const own = new Set<Address>()
    for (const line of lines) {
      ...
      const token = getAddress(event.token); seen.add(token)
      if (event.action === 'create') own.add(token)
    }
    trackedTokens = [...seen]; createdTokens = [...own]
  } catch (error: unknown) {
...
async function loadCostPositions(): Promise<CostPosition[]> {
  ...
  try {
    ...
    for (const line of lines) {
      ...
      const token = getAddress(event.token); const units = BigInt(event.tokenAmount)
      ...
      if (event.action === 'buy' || event.action === 'create') {
        ...
        if (Number.isFinite(costUsd) && costUsd > 0 && units > 0n) { position.units += units; position.costUsd += costUsd }
      } else if (event.action === 'sell' && units > 0n && position.units > 0n) {
        const sold = units > position.units ? position.units : units
        position.costUsd *= Number(position.units - sold) / Number(position.units); position.units -= sold
      }
...
async function quoteSell(token: Address, amount: bigint) {
  try {
    ...
    return BigInt(result.data || '0x0')
  } catch { console.log(`[SELL]   skipped unquotable position: ${token}`); return 0n }
}
...
async function closeLegacyPositions(stats: Stats) {
  ...
  for (const token of trackedTokens) {
    ...
    try {
      ...
      console.log(`[SELL]   closed legacy position ${token}: ~$${proceedsUsd.toFixed(2)} quoted, ${hash}`); closed++
    } catch (error) { console.log(`[SELL]   could not close legacy position ${token}: ${errorMessage(error)}`) }
  }
  return closed > 0
}
...
function mintedLpToken(receipt: { logs: readonly { address: Address; data: Hex; topics: readonly Hex[] }[] }): Address | undefined {
  ...
  for (const log of receipt.logs) {
    const from = log.topics[1]?.toLowerCase(); const to = log.topics[2]?.toLowerCase()
...
async function bondTrackedPosition(stats: Stats) {
  ...
  for (const token of candidates.slice(0, 1)) {
    try {
      ...
      await ensureTokenAllowance(token, tokenAmount); await ensureUsdcAllowance(usdcAmount)
      ...
      console.log(`[BOND]   bonded ${token}; LP position ${lpToken}: ${hash}`); return true
    } catch (error) { console.log(`[BOND]   skipped ${token}: ${errorMessage(error)}`) }
  }
  return false
}
...
async function unbondMaturedPosition(stats: Stats) {
  ...
  const positions = await loadBondPositions(); const seen = new Set<string>()
  for (const position of positions) {
    ...
    try {
      ...
      console.log(`[UNBOND] position closed: ${hash}`); return true
    } catch (error) { console.log(`[UNBOND] skipped ${position.token}: ${errorMessage(error)}`) }
  }
  return false
}

async function createLaunch(stats: Stats) {
  const name = randomName(); const symbol = randomSymbol(name)
  console.log(`\n[CREATE] ${name} ($${symbol})`)
  if (!config.dryRun) {
    ...
    trackedTokens = [...new Set([...trackedTokens, address])]; createdTokens = [...new Set([...createdTokens, address])]
    console.log(`  -> created ${address}: ${hash}`); return
  }
  if (!config.dryRun) requireContractIntegration('createLaunch')
  await pause(4_000, 9_000); await record(stats, 'create', { name, symbol }); console.log('  -> simulated create recorded')
}

async function buyOnCurve(stats: Stats, amountUsd: number) {
  console.log(`[BUY]    ~$${amountUsd.toFixed(2)}`)
  if (!config.dryRun) {
    if (stats.spentUsd + amountUsd > config.maxTotalSpend) { console.log(`[BUY]    spend cap reached; skipping this buy.`); return false }
    const availableUsd = await getFliptUsdcBalance(account.address); const spendableUsd = Math.max(0, availableUsd - 0.1)
    if (spendableUsd < 1.5) { console.log(`[BUY]    only ${availableUsd.toFixed(2)} Flipt USDC available; skipping.`); return false }
    ...
    for (const token of await discoverOtherLaunches()) {
      try {
        const balanceBefore = await tokenBalance(token); const data = encodeBuy(token, amount)
        await publicClient.call({ account: account.address, to: FLIPT.router, data })
        const hash = await walletClient.sendTransaction({ to: FLIPT.router, data })
        const receipt = await publicClient.waitForTransactionReceipt({ hash })
        ...
        console.log(`  -> buy confirmed: ${hash}`); return true
      } catch { console.log(`  -> skipped non-buyable launch: ${token}`) }
    }
    console.log('  -> no currently buyable recent launch found; continuing cycle.'); return false
  }
  if (!config.dryRun) requireContractIntegration('buyOnCurve')
  await pause(3_000, 7_000); await record(stats, 'buy', { amountUsd: Number(amountUsd.toFixed(2)) }); console.log('  -> simulated buy recorded')
}
...
async function managePosition(stats: Stats) {
  ...
  if (Math.random() > 0.65) {
    ...
    await pause(2_000, 5_000); await record(stats, 'unbond')
  } else { console.log('[HOLD]   staying bonded'); await record(stats, 'hold') }
}
...
async function main() {
  ...
  try {
    ...
    while (!stopping) {
      ...
      try {
        touchRunner('checking-balances')
        const [balance, fliptUsdc] = await Promise.all([
          getBalance(account.address), getFliptUsdcBalance(account.address),
        ])
        stats.nativeGasUsdc = balance; stats.fliptUsdc = fliptUsdc
        ...
        // A failed create no longer escapes main(). The next cycle generates a
        // fresh name/symbol/salt and retries after recovery backoff.
        touchRunner('creating-launch')
        try {
          await createLaunch(stats)
        } catch (error) {
          throw new Error(`createLaunch failed: ${errorMessage(error)}`, { cause: error })
        }

        if (paused || stopping) continue
        touchRunner('post-create-wait')
        await pause(20_000, 60_000)

        const buyCount = rand(1, 3)
        for (let index = 0; index < buyCount && !stopping; index += 1) {
          if (paused) break
          touchRunner(`buy-${index + 1}-of-${buyCount}`)
          try {
            await buyOnCurve(stats, randFloat(config.minBuy, config.maxBuy))
          } catch (error) {
            console.error(`[BUY ERROR] ${errorMessage(error)}`)
            lastRunnerError = `Buy failed: ${errorMessage(error)}`
            lastRunnerErrorAt = new Date().toISOString()
            // Buy failure is non-fatal to the cycle.
          }
          if (!paused && !stopping) await pause(15_000, 50_000)
        }

        if (!paused && !stopping) {
          touchRunner('managing-position')
          try {
            await managePosition(stats)
          } catch (error) {
            console.error(`[MANAGE ERROR] ${errorMessage(error)}`)
            lastRunnerError = `Position management failed: ${errorMessage(error)}`
            lastRunnerErrorAt = new Date().toISOString()
            // Management failure is non-fatal to the runner.
          }
        }
        ...
        consecutiveErrors = 0
        touchRunner('cycle-complete')
        console.log(`Cycle ${stats.cycles} done. Totals: ${stats.create} creates, ${stats.buy} buys.`)

        if (config.maxCycles > 0 && stats.cycles >= config.maxCycles) {
          console.log(`MAX_CYCLES (${config.maxCycles}) reached.`)
          break
        }

        touchRunner('sleeping-between-cycles')
        console.log('Sleeping 1 minute(s)...\n')
        await pause(60_000, 60_000)
      } catch (error) {
        const message = errorMessage(error)
        consecutiveErrors += 1
        lastRunnerError = message
        lastRunnerErrorAt = new Date().toISOString()
        touchRunner('recovering')
        console.error('\n[CYCLE ERROR]')
        console.error(error)

        const backoffMs = Math.min(30_000 * consecutiveErrors, 5 * 60_000)
        console.log(`[RECOVERY] ${message}`)
        console.log(`[RECOVERY] Runner remains alive. Retry ${consecutiveErrors} in ${Math.round(backoffMs / 1000)} seconds.`)
        void telegramNotify(`[RECOVERY] Trading cycle failed but runner is still alive.\n${message}\nRetrying in ${Math.round(backoffMs / 1000)}s.`)

        // Use plain sleep here: recovery backoff should not be shortened by FAST_MODE.
        await sleep(backoffMs)
      }
    }
  } finally {
    runnerAlive = false
    touchRunner(stopping ? 'stopped' : 'exited')
  }
}
...
createServer((request, response) => {
  if (request.url === '/health' || request.url === '/status') {
    const healthy = runnerAlive && !stopping
  ...
  response.end('Flipt bot service is running. Use Telegram /status for details.\n')
...
main().catch((error: unknown) => {
  runnerAlive = false
  lastRunnerError = `Fatal runner error: ${errorMessage(error)}`
  lastRunnerErrorAt = new Date().toISOString()
  touchRunner('fatal')
  console.error('[FATAL RUNNER ERROR]', error)
  void telegramNotify(`[FATAL] Trading runner exited.\n${lastRunnerError}`)
  process.exitCode = 1
})
